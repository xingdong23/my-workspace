...
如果你准备把 OpenClaw 的源码定制也写进教程，最值得总结的其实不是“我改了哪些文件”，而是：

- 什么时候该改源码
- 源码该改哪一层
- 哪几类问题最值得进入源码层解决

从实战角度看，最常见、也最有代表性的源码级定制，其实就三类。

### 第一类：渠道适配层

典型代表就是飞书共享群里的多 bot 提及路由。
...
这时候问题往往不在模型，也不在 prompt，而在渠道插件如何：
- 解析 mention
- 构造入站上下文
- 做消息路由
...
- `/v1/models` 正常
- API Key 正常
- 但 OpenClaw 一走真实 agent 请求就报 `403`

这时候真正该查的，不是“模型是不是不行”，而是：
- OpenClaw 真实发了哪些 headers
- OpenClaw 真实 body 和手写请求差在哪
- 是哪个 provider/代理站在拦 SDK 指纹
...
本质上它不是业务逻辑调整，而是**兼容层补丁**。

### 第三类：安全策略层

这一类通常发生在本机网络环境比较复杂的时候，比如：
- Clash
- Mihomo
- Surge
- fake-ip
...
典型现象是：
- 你访问的是公网域名
- 但 OpenClaw 却报 `Blocked: resolves to private/internal/special-use IP address`

这时候问题不在网页本身，而在：
- 本地代理把公网域名解析成 fake-ip
- OpenClaw 的 SSRF 防护把它当成内网地址拦掉了
...
它不是功能增强，而是**安全边界调整**，所以一定要谨慎，也一定要留文档。

### 这一章最重要的结论

源码级定制最有价值的，不是“把所有东西都改成自己喜欢的样子”，而是只在下面三种情况下出手：

1. 渠道适配层确实丢了关键信息
2. Provider 兼容层确实存在真实请求形态问题
3. 安全策略层确实和你的运行环境发生了冲突

也就是说，先把问题收缩到“哪一层坏了”，再决定改哪一层。

---

## 第 15 章：高级用户必须接受的一件事，升级会覆盖你的补丁
...
讲什么时候该改源码、源码该改哪一层、三类最常见的源码级定制场景，以及改完后怎么验证、怎么维护。
